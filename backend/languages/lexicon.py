...
JAPANESE_LEXICON: Dict[str, Dict[str, str]] = {
    ...
    "会って": {"pt": "encontrando", "en": "meeting", "pos": "VERB"},
...
def is_invalid_translation(
    ...
    # 2. Placeholders genéricos e resquícios de fallbacks
...
def get_auxiliary_translation(
    ...
    if lang_code in ["zh", "chinese", "mandarin"]:
        ...
        # Decomposição morfológica apenas para palavras curtas de 2 a 3 caracteres onde TODOS os caracteres possuem sentido no léxico
        if len(clean) in [2, 3]:
            parts = []
            matched = 0
            for char in clean:
            ...
            if matched == len(clean) and parts:
                return " | ".join(parts)
        ...
        # Tentativa com remoção de partículas gramaticais comuns anexadas ao final (ex: レストランに -> レストラン)
        particles = ["からは", "までは", "には", "では", "から", "まで", "より", "など", "に", "で", "を", "は", "が", "の", "と", "へ", "も", "よ", "ね", "か"]
        for p in particles:
            if clean.endswith(p) and len(clean) > len(p):
                stem = clean[:-len(p)]
                stem_entry = JAPANESE_LEXICON.get(stem)
                if stem_entry:
                    stem_trans = stem_entry.get(target_key) or stem_entry.get("pt")
                    p_entry = JAPANESE_LEXICON.get(p)
                    if p_entry:
                        p_gloss = (p_entry.get(target_key) or p_entry.get("pt") or "").split("/")[0].strip()
                        return f"{stem_trans} + [{p}: {p_gloss}]"
                    return stem_trans
    ...
    if lang_code in ["zh", "chinese", "mandarin"]:
        ...
        for p in particles:
            if clean.endswith(p) and len(clean) > len(p):
                stem = clean[:-len(p)]
                stem_entry = JAPANESE_LEXICON.get(stem)
                if stem_entry and stem_entry.get("pos"):
                    return stem_entry.get("pos")
